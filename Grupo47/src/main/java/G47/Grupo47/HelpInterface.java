...
public class HelpInterface extends Shell {
	...
	public HelpInterface(Display display) {
		...
		lblHelpExtract.setText("1-Selecione a pasta onde se localiza o projeto do qual pretende extrair métricas, clicando no botão 'Selecionar pasta'. De seguida, pressione o botão 'Extrair métricas', do qual resultará um ficheiro com as respetivas métricas calculadas.");
		...
		lblExtractResult.setText("2-Pode agora verificar que, na lista centrada à esqueda da interface, foi gerada uma lista com o nome do(s) ficheiro(s) excel gerado(s). Ao clicar sobre esse ficheiro, poderá verificar as características gerais do mesmo, nos campos localizados à direita da interface. ");
		
		//INSTRUÇÃO 3-Definir regras
		Label lblRules = new Label(this, SWT.BORDER | SWT.WRAP);
		lblRules.setBounds(10, 147, 870, 66);
		lblRules.setText("3-Após este processo de extração, poderá agora definir regras para a deteção de codesmells. Para tal, preencha os campos que se encontram vazios de forma válida e coerente, e de seguida pressione o botão 'Definir regra'. De recordar que não há um limite de regras por definir.");
		
		//INSTRUÇÃO 4-Alterar regras
		Label lblChangeRules = new Label(this, SWT.BORDER | SWT.WRAP);
		lblChangeRules.setBounds(10, 219, 870, 66);
		lblChangeRules.setText("4- Há medida que as regras são definidas, poderá notar que as mesmas são guardadas numa lista localizada no canto inferior esquerdo da interface. Se pretender alterar alguma destas regras, basta seleciona-la, alterar os campos desejados no mesmo sitio onde os definiu e, após isto, clicar no botão 'Alterar regra'.");
		...
		lblCodesmellsDetect.setText("6- Por fim, temos a deteção dos codesmells. Para tal, basta selecionar uma das regras resultantes da sua definição, para a qual pretende fazer esta mesma deteção. Após esta seleção, pressione o botão 'Deteção de codesmells', da qual resultará uma nova interface com os resultados obtidos.");
